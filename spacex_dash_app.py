...
def get_pie_chart(entered_site):
    if entered_site == 'ALL':
        ...
        names='Launch Site', 
        title='Successful Launch Count per Site')
        return fig
    else:
...
def get_scatter_chart(entered_site, entered_payload):
    payload_range_df = spacex_df[(spacex_df['Payload Mass (kg)'] >= entered_payload[0]) & (spacex_df['Payload Mass (kg)'] <= entered_payload[1])]
    if entered_site == 'ALL':
